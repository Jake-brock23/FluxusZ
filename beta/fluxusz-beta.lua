--[[
Changed ScreenGui parent from CoreGui -> gethui()
Added Kanami / Detectedly internal functions
  maketoast
  pushautoexec
...
-- PLEASE COPY THIS WHEN YOU'RE GONNA PUSH A NEW UI STARTING HERE
local createToast
if dtc and dtc.maketoast then
	createToast = dtc.maketoast
end

if _G.Loaded then
	createToast("Fluxus Z is already running!")
	return
end
		...
		if tbox and tbox:IsA("TextBox") then
			Button.MouseButton1Click:Connect(function()
				...
				if not success then
					...
					createToast("Error executing code:\n" .. tostring(result))
				else
					createToast("Script executed successfully")
				end
			end)
		end
	end
